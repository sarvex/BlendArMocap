...
class CgtMPHandOutNode(mp_out_utils.BpyOutputNode):
    ...
    def __init__(self):
        ...
        cgt_collection.create_collection(f"{self.col_name}S", self.parent_col)
        cgt_collection.create_collection(f"{self.col_name}.L", f"{self.col_name}S")
        cgt_collection.create_collection(f"{self.col_name}.R", f"{self.col_name}S")
        cgt_collection.add_list_to_collection(
            f"{self.col_name}.L", self.left_hand, self.parent_col
        )
        cgt_collection.add_list_to_collection(
            f"{self.col_name}.R", self.right_hand, self.parent_col
        )
